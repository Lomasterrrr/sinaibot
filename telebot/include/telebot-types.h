...
typedef struct telebot_user {
	...
	/** Optional. Optional. True, if the bot can be invited to groups.
...
typedef struct telebot_chat {
	...
	/** Type of chat, can be either "private", or "group", "supergroup", or
	 * "channel". */
	char *type;
...
typedef struct telebot_message {
	...
	struct telebot_successful_payment
	    *successful_payment; // TODO:define type
...
typedef struct telebot_audio {
	...
	/** Optional. Thumbnail of the album cover to which the music file
	 * belongs */
 ...
 * the button. Optional fields request_contact, request_location, and
...
typedef struct telebot_chat_member {
	...
	/**
	 * Optional. Restricted only. True, if the user is a member of the chat
	...
	/**
	 * Optional. Restricted only. True, if the user is allowed to add web
...
typedef struct telebot_update {
	...
	union {
		...
		/** New version of a channel post that is known to the bot and
		 * was edited */
		...
		/** New incoming shipping query. Only for invoices with flexible
		 * price */
		// TODO: telebot_shipping_query_t shipping_query;

		/** New incoming pre-checkout query. Contains full information
		 * about checkout */
...
/**
 * @brief Thi object represetns information about the current status of a
 * webhook.
 */
