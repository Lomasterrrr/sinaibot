 ...
 * MUST have corresponding call to #telebot_destroy() when operation is
 * complete.
 *
 ...
 * @param[in] allowed_updates An array of types of updates you want your bot to
 * receive. Refers to #telebot_update_type_e.
 ...
 * @return On success, #TELEBOT_ERROR_NONE is returned, and webhook information
 * is stored in input parameter.
 */
 ...
 * sending voice messages, use the telegram_send_voice() function instead.
 * @param[in] handle The telebot handler created with #telebot_create().
 * @param[in] chat_id Unique identifier for the target chat or username of the
 * target channel (in the format \@channelusername).
 ...
 * @param[in] options A aray of answer options, 2-10 strings 1-100 characters
 * each.
...
telebot_error_e telebot_send_poll(telebot_handler_t handle,
    ...
    int count_options, bool is_anonymous, const char *type,
 ...
 * #telebot_send_chat_action() with action = upload_photo. The user will see a
 * "sending photo" status for the bot. It is only recommended to use when a
...
telebot_error_e telebot_download_file(telebot_handler_t handle,
    const char *file_id, const char *path);
...
telebot_error_e telebot_get_chat(telebot_handler_t handle,
    long long int chat_id, telebot_chat_t *chat);
...
telebot_error_e telebot_get_chat_admins(telebot_handler_t handle,
    long long int chat_id, telebot_chat_member_t **admin, int *count);
 ...
 * #telebot_get_chat requests to check if the bot can use it.
 * @param[in] handle The telebot handler created with #telebot_create().
 * @param[in] chat_id Unique identifier for the target chat or username of the
 * target channel (in the format \@channelusername).
 ...
 * administrator in the chat for this to work and must have the appropriate
 * admin rights. Use the field can_set_sticker_set optionally returned in
 * #telebot_get_chat requests to check if the bot can use this method.
 * @param[in] handle The telebot handler created with #telebot_create().
 * @param[in] chat_id Unique identifier for the target chat or username of the
 * target channel (in the format \@channelusername).
 * @return on Success, #TELEBOT_ERROR_NONE is returned, otherwise a negative
