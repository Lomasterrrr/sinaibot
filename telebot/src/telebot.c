...
telebot_error_e
telebot_create(telebot_handler_t *handle, char *token)
{
	if ((token == NULL) || (handle == NULL))
		return TELEBOT_ERROR_INVALID_PARAMETER;

	telebot_hdata_t *_handle = (telebot_hdata_t *)calloc(1,
	    sizeof(telebot_hdata_t));
	if (_handle == NULL) {
...
telebot_error_e
telebot_destroy(telebot_handler_t handle)
{
...
telebot_error_e
telebot_set_proxy(telebot_handler_t handle, char *addr, char *auth)
{
	if (addr == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_get_proxy(telebot_handler_t handle, char **addr)
{
	if (addr == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_get_updates(telebot_handler_t handle, int offset, int limit,
    int timeout, telebot_update_type_e allowed_updates[],
    int allowed_updates_count, telebot_update_t **updates, int *count)
{
	...
	*count = 0;
	...
	int ret = telebot_core_get_updates(_handle->core_h, _offset, _limit,
	    _timeout, str_allowed_updates, &response);
	if (ret != TELEBOT_ERROR_NONE) {
		if (array)
...
telebot_error_e
telebot_put_updates(telebot_update_t *updates, int count)
{
	...
	for (int index = 0; index < count; index++) {
		...
		case TELEBOT_UPDATE_TYPE_EDITED_CHANNEL_POST:
			telebot_put_message(
			    &(updates[index].edited_channel_post));
			break;
		case TELEBOT_UPDATE_TYPE_CALLBACK_QUERY:
			telebot_put_callback_query(
			    &(updates[index].callback_query));
			break;
		...
		default:
			ERR("Unsupported update type: %d",
			    updates[index].update_type);
		}
	}
...
telebot_error_e
telebot_get_me(telebot_handler_t handle, telebot_user_t *me)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
...
telebot_error_e
telebot_put_me(telebot_user_t *me)
{
	if (me == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_set_webhook(telebot_handler_t handle, char *url, char *certificate,
    ...
    int allowed_updates_count)
{
	...
	if (allowed_updates_count > 0) {
		...
		for (i = 0; i < allowed_updates_count; i++) {
			...
			if (i <
			    (allowed_updates_count - 1)) // intermediate element
				strncat(allowed_updates_str, ",",
				    TELEBOT_BUFFER_BLOCK);
		}
		strncat(allowed_updates_str, "]", TELEBOT_BUFFER_BLOCK);
...
telebot_error_e
telebot_delete_webhook(telebot_handler_t handle)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
...
telebot_error_e
telebot_get_webhook_info(telebot_handler_t handle, telebot_webhook_info_t *info)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
...
telebot_error_e
telebot_put_webhook_info(telebot_webhook_info_t *info)
{
	if (info == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_send_message(telebot_handler_t handle, long long int chat_id,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_message(_handle->core_h, chat_id, text,
...
telebot_error_e
telebot_forward_message(telebot_handler_t handle, long long int chat_id,
    long long int from_chat_id, bool disable_notification, int message_id)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_forward_message(_handle->core_h, chat_id,
	    from_chat_id, disable_notification, message_id, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_photo(telebot_handler_t handle, long long int chat_id,
    const char *photo, bool is_file, const char *caption,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_photo(_handle->core_h, chat_id, photo,
	    is_file, caption, parse_mode, disable_notification,
	    reply_to_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_audio(telebot_handler_t handle, long long int chat_id,
    const char *audio, bool is_file, const char *caption,
    const char *parse_mode, int duration, const char *performer,
    ...
    int reply_to_message_id, const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_audio(_handle->core_h, chat_id, audio,
	    is_file, caption, parse_mode, duration, performer, title, thumb,
...
telebot_error_e
telebot_send_document(telebot_handler_t handle, long long int chat_id,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_document(_handle->core_h, chat_id, document,
	    is_file, thumb, caption, parse_mode, disable_notification,
	    reply_to_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_video(telebot_handler_t handle, long long int chat_id,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_video(_handle->core_h, chat_id, video,
	    is_file, duration, width, height, thumb, caption, parse_mode,
...
telebot_error_e
telebot_send_animation(telebot_handler_t handle, long long int chat_id,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_animation(_handle->core_h, chat_id,
	    animation, is_file, duration, width, height, thumb, caption,
	    parse_mode, disable_notification, reply_to_message_id, reply_markup,
	    &response);
	if (ret != TELEBOT_ERROR_NONE)
		return ret;
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_voice(telebot_handler_t handle, long long int chat_id,
    const char *voice, bool is_file, const char *caption,
    ...
    int reply_to_message_id, const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_voice(_handle->core_h, chat_id, voice,
	    is_file, caption, parse_mode, duration, disable_notification,
	    reply_to_message_id, reply_markup, &response);
	if (ret != TELEBOT_ERROR_NONE)
		return ret;
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_video_note(telebot_handler_t handle, long long int chat_id,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_video_note(_handle->core_h, chat_id,
	    video_note, is_file, duration, length, thumb, disable_notification,
	    reply_to_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_location(telebot_handler_t handle, long long int chat_id,
    ...
    int reply_to_message_id, const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_location(_handle->core_h, chat_id, latitude,
	    longitude, live_period, disable_notification, reply_to_message_id,
	    reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_edit_message_live_location(telebot_handler_t handle,
    ...
    float latitude, float longitude, const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_edit_message_live_location(_handle->core_h,
...
telebot_error_e
telebot_stop_message_live_location(telebot_handler_t handle,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_stop_message_live_location(_handle->core_h,
	    chat_id, message_id, inline_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_venue(telebot_handler_t handle, long long int chat_id,
    float latitude, float longitude, const char *title, const char *address,
    const char *foursquare_id, const char *foursquare_type,
    bool disable_notification, int reply_to_message_id,
    const char *reply_markup)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_venue(_handle->core_h, chat_id, latitude,
	    longitude, title, address, foursquare_id, foursquare_type,
	    disable_notification, reply_to_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_contact(telebot_handler_t handle, long long int chat_id,
    ...
    const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_contact(_handle->core_h, chat_id,
...
telebot_error_e
telebot_send_poll(telebot_handler_t handle, long long int chat_id,
    const char *question, const char **options, int count_options,
...
telebot_error_e
telebot_send_dice(telebot_handler_t handle, long long int chat_id,
    bool disable_notification, int reply_to_message_id,
    const char *reply_markup)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_dice(_handle->core_h, chat_id,
	    disable_notification, reply_to_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_send_chat_action(telebot_handler_t handle, long long int chat_id,
    char *action)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
	...
	telebot_core_response_t response;
	int ret = telebot_core_send_chat_action(_handle->core_h, chat_id,
	    action, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_get_user_profile_photos(telebot_handler_t handle, long long int user_id,
    int offset, int limit, telebot_user_profile_photos_t *photos)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_get_user_profile_photos(_handle->core_h, user_id,
...
telebot_error_e
telebot_put_user_profile_photos(telebot_user_profile_photos_t *photos)
{
	if (photos == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_download_file(telebot_handler_t handle, const char *file_id,
    const char *path)
{
	telebot_file_t file;
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
...
telebot_error_e
telebot_kick_chat_member(telebot_handler_t handle, long long int chat_id,
    long long int user_id, long until_date)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_kick_chat_member(_handle->core_h, chat_id,
	    user_id, until_date, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_unban_chat_member(telebot_handler_t handle, long long int chat_id,
    long long int user_id)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_unban_chat_member(_handle->core_h, chat_id,
	    user_id, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_restrict_chat_member(telebot_handler_t handle, long long int chat_id,
    long long int user_id, long until_date, bool can_send_messages,
    bool can_send_media_messages, bool can_send_polls,
    ...
    bool can_change_info, bool can_invite_users, bool can_pin_messages)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_restrict_chat_member(_handle->core_h, chat_id,
	    user_id, until_date, can_send_messages, can_send_media_messages,
	    can_send_polls, can_send_other_messages, can_add_web_page_previews,
	    can_change_info, can_invite_users, can_pin_messages, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_promote_chat_member(telebot_handler_t handle, long long int chat_id,
    long long int user_id, bool can_change_info, bool can_post_messages,
    ...
    bool can_restrict_members, bool can_pin_messages, bool can_promote_members)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_promote_chat_member(_handle->core_h, chat_id,
	    user_id, can_change_info, can_post_messages, can_edit_messages,
	    can_delete_messages, can_invite_users, can_restrict_members,
	    can_pin_messages, can_promote_members, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_set_chat_admin_custom_title(telebot_handler_t handle,
    long long int chat_id, long long int user_id, const char *custom_title)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_set_chat_admin_custom_title(_handle->core_h,
	    chat_id, user_id, custom_title, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_set_chat_permissions(telebot_handler_t handle, long long int chat_id,
...
telebot_error_e
telebot_export_chat_invite_link(telebot_handler_t handle, long long int chat_id,
    char **invite_link)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
	...
	*invite_link = NULL;
	...
	telebot_core_response_t response;
	int ret = telebot_core_export_chat_invite_link(_handle->core_h, chat_id,
	    &response);
	if (ret != TELEBOT_ERROR_NONE)
		return ret;
	...
	struct json_object *result = NULL;
	if (json_object_object_get_ex(obj, "result", &result)) {
		*invite_link = TELEBOT_SAFE_STRDUP(
...
telebot_error_e
telebot_set_chat_photo(telebot_handler_t handle, long long int chat_id,
    const char *photo)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
	...
	telebot_core_response_t response;
	int ret = telebot_core_set_chat_photo(_handle->core_h, chat_id, photo,
	    &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_delete_chat_photo(telebot_handler_t handle, long long int chat_id)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
	...
	telebot_core_response_t response;
	int ret = telebot_core_delete_chat_photo(_handle->core_h, chat_id,
	    &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_set_chat_title(telebot_handler_t handle, long long int chat_id,
    const char *title)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
	...
	telebot_core_response_t response;
	int ret = telebot_core_set_chat_title(_handle->core_h, chat_id, title,
	    &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_set_chat_description(telebot_handler_t handle, long long int chat_id,
    const char *description)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_set_chat_description(_handle->core_h, chat_id,
...
telebot_error_e
telebot_pin_chat_message(telebot_handler_t handle, long long int chat_id,
    int message_id, bool disable_notification)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_pin_chat_message(_handle->core_h, chat_id,
	    message_id, disable_notification, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_unpin_chat_message(telebot_handler_t handle, long long int chat_id)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
	...
	telebot_core_response_t response;
	int ret = telebot_core_unpin_chat_message(_handle->core_h, chat_id,
	    &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_leave_chat(telebot_handler_t handle, long long int chat_id)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
...
telebot_error_e
telebot_get_chat(telebot_handler_t handle, long long int chat_id,
    telebot_chat_t *chat)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
...
telebot_error_e
telebot_get_chat_admins(telebot_handler_t handle, long long int chat_id,
    telebot_chat_member_t **admins, int *count)
{
	...
	*count = 0;
	...
	telebot_core_response_t response;
	int ret = telebot_core_get_chat_admins(_handle->core_h, chat_id,
	    &response);
	if (ret != TELEBOT_ERROR_NONE)
		return ret;
...
telebot_error_e
telebot_put_chat_admins(telebot_chat_member_t *admins, int count)
{
	if (admins == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_get_chat_members_count(telebot_handler_t handle, long long int chat_id,
    int *count)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
	...
	*count = 0;
	...
	telebot_core_response_t response;
	int ret = telebot_core_get_chat_members_count(_handle->core_h, chat_id,
	    &response);
	if (ret != TELEBOT_ERROR_NONE)
		return ret;
...
telebot_error_e
telebot_get_chat_member(telebot_handler_t handle, long long int chat_id,
    long long int user_id, telebot_chat_member_t *member)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_get_chat_member(_handle->core_h, chat_id,
	    user_id, &response);
	if (ret != TELEBOT_ERROR_NONE)
		return ret;
...
telebot_error_e
telebot_put_chat_member(telebot_chat_member_t *member)
{
	if (member == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_set_chat_sticker_set(telebot_handler_t handle, long long int chat_id,
...
telebot_error_e
telebot_delete_chat_sticker_set(telebot_handler_t handle, long long int chat_id)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
	...
	telebot_core_response_t response;
	int ret = telebot_core_delete_chat_sticker_set(_handle->core_h, chat_id,
	    &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_answer_callback_query(telebot_handler_t handle,
    ...
    const char *url, int cache_time)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_answer_callback_query(_handle->core_h,
	    callback_query_id, text, show_alert, url, cache_time, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_set_my_commands(telebot_handler_t handle,
    telebot_bot_command_t commands[], int count)
{
	...
	for (int i = 0; i < count; i++) {
		...
		json_object_object_add(obj, "command",
		    json_object_new_string(commands[i].command));
		json_object_object_add(obj, "description",
		    json_object_new_string(commands[i].description));
	...
	telebot_core_response_t response;
	int ret = telebot_core_set_my_commands(_handle->core_h, array_options,
	    &response);
	telebot_core_put_response(&response);
	json_object_put(array);
...
telebot_error_e
telebot_get_my_commands(telebot_handler_t handle,
    telebot_bot_command_t **commands, int *count)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
...
telebot_error_e
telebot_put_my_commands(telebot_bot_command_t *commands, int count)
{
	if ((commands == NULL) || (count <= 0))
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_edit_message_text(telebot_handler_t handle, long long int chat_id,
    int message_id, const char *inline_message_id, const char *text,
    const char *parse_mode, bool disable_web_page_preview,
    const char *reply_markup)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
	...
	telebot_core_response_t response;
	int ret = telebot_core_edit_message_text(_handle->core_h, chat_id,
	    message_id, inline_message_id, text, parse_mode,
	    disable_web_page_preview, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_edit_message_caption(telebot_handler_t handle, long long int chat_id,
    int message_id, const char *inline_message_id, const char *caption,
    const char *parse_mode, const char *reply_markup)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
		return TELEBOT_ERROR_NOT_SUPPORTED;
	...
	telebot_core_response_t response;
	int ret = telebot_core_edit_message_caption(_handle->core_h, chat_id,
...
telebot_error_e
telebot_edit_message_reply_markup(telebot_handler_t handle,
    long long int chat_id, int message_id, const char *inline_message_id,
    const char *reply_markup)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	...
	telebot_core_response_t response;
	int ret = telebot_core_edit_message_reply_markup(_handle->core_h,
	    chat_id, message_id, inline_message_id, reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_stop_poll(telebot_handler_t handle, long long int chat_id,
    int message_id, const char *reply_markup)
{
	...
	telebot_core_response_t response;
	int ret = telebot_core_stop_poll(_handle->core_h, chat_id, message_id,
	    reply_markup, &response);
	telebot_core_put_response(&response);
...
telebot_error_e
telebot_delete_message(telebot_handler_t handle, long long int chat_id,
    int message_id)
{
	telebot_hdata_t *_handle = (telebot_hdata_t *)handle;
	if (_handle == NULL)
...
static void
telebot_put_user(telebot_user_t *user)
{
	if (user == NULL)
		return;
...
telebot_error_e
telebot_put_chat(telebot_chat_t *chat)
{
	if (chat == NULL)
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
static void
telebot_put_chat_photo(telebot_chat_photo_t *photo)
{
	if (photo == NULL)
		return;
...
static void
telebot_put_chat_permissions(telebot_chat_permissions_t *permissions)
{
	if (permissions == NULL)
		return;
	// Nothing for now
...
static void
telebot_put_message(telebot_message_t *msg)
{
	if (msg == NULL)
		return;
	...
	if (msg->entities) {
		for (int index = 0; index < msg->count_entities; index++)
			telebot_put_telebot_message_entity(
			    &(msg->entities[index]));
	...
	if (msg->caption_entities) {
		for (int index = 0; index < msg->count_caption_entities;
		    index++)
			telebot_put_telebot_message_entity(
			    &(msg->caption_entities[index]));
	...
	if (msg->new_chat_members) {
		for (int index = 0; index < msg->count_new_chat_members;
		    index++)
	...
	if (msg->left_chat_members) {
		for (int index = 0; index < msg->count_left_chat_members;
		    index++)
...
static void
telebot_put_telebot_message_entity(telebot_message_entity_t *entity)
{
	if (entity == NULL)
		return;
...
static void
telebot_put_photo(telebot_photo_t *photo)
{
	if (photo == NULL)
		return;
...
static void
telebot_put_audio(telebot_audio_t *audio)
{
	if (audio == NULL)
		return;
...
static void
telebot_put_document(telebot_document_t *document)
{
	if (document == NULL)
		return;
...
static void
telebot_put_video(telebot_video_t *video)
{
	if (video == NULL)
		return;
...
static void
telebot_put_animation(telebot_animation_t *animation)
{
	if (animation == NULL)
		return;
...
static void
telebot_put_voice(telebot_voice_t *voice)
{
	if (voice == NULL)
		return;
...
static void
telebot_put_video_note(telebot_video_note_t *vnote)
{
	if (vnote == NULL)
		return;
...
static void
telebot_put_contact(telebot_contact_t *contact)
{
	if (contact == NULL)
		return;
...
static void
telebot_put_location(telebot_location_t *location)
{
	if (location == NULL)
		return;
	// Nothing to free
...
static void
telebot_put_venue(telebot_venue_t *venue)
{
	if (venue == NULL)
		return;
...
static void
telebot_put_file(telebot_file_t *file)
{
	if (file == NULL)
		return;
...
static void
telebot_put_poll_option(telebot_poll_option_t *poll_option)
{
	if (poll_option == NULL)
		return;
...
static void
telebot_put_poll_answer(telebot_poll_answer_t *answer)
{
	if (answer == NULL)
		return;
...
static void
telebot_put_poll(telebot_poll_t *poll)
{
	if (poll == NULL)
		return;
...
static void
telebot_put_dice(telebot_dice_t *dice)
{
	if (dice == NULL)
		return;
	// Nothing to free
...
static void
telebot_put_callback_query(telebot_callback_query_t *query)
{
	if (query == NULL)
		return;
