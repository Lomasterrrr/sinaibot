...
void
telebot_core_put_response(telebot_core_response_t *response)
{
	if (response) {
...
static size_t
write_data_cb(void *contents, size_t size, size_t nmemb, void *userp)
{
...
telebot_core_curl_perform(telebot_core_handler_t *core_h, const char *method,
    telebot_core_mime_t mimes[], size_t size, telebot_core_response_t *resp)
{
	CURLcode res;
	CURL *curl_h = NULL;
	curl_mime *mime = NULL;
	...
	if (core_h->proxy_addr != NULL) {
		...
		if (core_h->proxy_auth != NULL) {
			curl_easy_setopt(curl_h, CURLOPT_PROXYAUTH,
			    CURLAUTH_ANY);
			curl_easy_setopt(curl_h, CURLOPT_PROXYUSERPWD,
			    core_h->proxy_auth);
		}
	}
	...
	if (size > 0) {
		...
		for (int index = 0; index < size; index++) {
			...
			if (mimes[index].type == TELEBOT_MIME_TYPE_FILE)
				...
				curl_mime_data(part, mimes[index].data,
	...
	if (resp_code != 200L) {
		...
		if (obj == NULL) {
			...
			ERR("HTTP error: %d - \"%s\"",
			    json_object_get_int(
				json_object_object_get(obj, "error_code")),
			    json_object_get_string(
				json_object_object_get(obj, "description")));
		}
...
telebot_core_create(telebot_core_handler_t **core_h, const char *token)
{
	...
	*core_h = NULL;
	...
	telebot_core_handler_t *_core_h = calloc(1,
	    sizeof(telebot_core_handler_t));
	if (_core_h == NULL) {
...
telebot_core_destroy(telebot_core_handler_t **core_h)
{
	...
	if ((*core_h)->proxy_addr != NULL) {
		memset((*core_h)->proxy_addr, 0xFF,
		    strlen((*core_h)->proxy_addr));
		TELEBOT_SAFE_FREE((*core_h)->proxy_addr);
	}

	if ((*core_h)->proxy_auth != NULL) {
		memset((*core_h)->proxy_auth, 0xFF,
		    strlen((*core_h)->proxy_auth));
...
telebot_error_e
telebot_core_set_proxy(telebot_core_handler_t *core_h, const char *addr,
    const char *auth)
{
...
telebot_core_get_updates(telebot_core_handler_t *core_h, int offset, int limit,
    int timeout, const char *allowed_updates, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_UPDATES,
	    mimes, index, response);
}

telebot_error_e
telebot_core_set_webhook(telebot_core_handler_t *core_h, const char *url,
    ...
    telebot_core_response_t *response)
{
	...
	mimes[index].type = TELEBOT_MIME_TYPE_DATA;
	snprintf(mimes[index].data, sizeof(mimes[index].data), "%d",
	    max_connections);
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SET_WEBHOOK,
	    mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_DELETE_WEBHOOK,
	    NULL, 0, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_GET_WEBHOOK_INFO, NULL, 0, response);
}
...
telebot_error_e
telebot_core_send_message(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_MESSAGE,
	    mimes, index, response);
}

telebot_error_e
telebot_core_forward_message(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	mimes[index].type = TELEBOT_MIME_TYPE_DATA;
	snprintf(mimes[index].data, sizeof(mimes[index].data), "%lld",
	    from_chat_id);
	++index;
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_FORWARD_MESSAGE,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_photo(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	mimes[index].type = (is_file ? TELEBOT_MIME_TYPE_FILE :
				       TELEBOT_MIME_TYPE_DATA);
	...
	if (caption != NULL) {
		...
		if (parse_mode != NULL) {
			mimes[index].name = "parse_mode";
			mimes[index].type = TELEBOT_MIME_TYPE_DATA;
			snprintf(mimes[index].data, sizeof(mimes[index].data),
			    "%s", parse_mode);
			++index;
		}
	}
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_PHOTO,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_audio(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_AUDIO,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_document(telebot_core_handler_t *core_h,
...

{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_DOCUMENT,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_video(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_VIDEO,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_animation(telebot_core_handler_t *core_h,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_ANIMATION,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_voice(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_VOICE,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_video_note(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_VIDEO_NOTE,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_location(telebot_core_handler_t *core_h,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_LOCATION,
	    mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_EDIT_MESSAGE_LIVE_LOCATION, mimes, index, response);
}
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_STOP_MESSAGE_LIVE_LOCATION, mimes, index, response);
}

telebot_error_e
telebot_core_send_venue(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_VENUE,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_contact(telebot_core_handler_t *core_h, long long int chat_id,
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_CONTACT,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_poll(telebot_core_handler_t *core_h, long long int chat_id,
    const char *question, const char *options, bool is_anonymous,
    const char *type, bool allows_multiple_answers, int correct_option_id,
    bool is_closed, bool disable_notification, int reply_to_message_id,
    const char *reply_markup, telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL) || (question == NULL) ||
	    (options == NULL)) {
		ERR("Handler, token, phone_number, or first_name is NULL");
		return TELEBOT_ERROR_INVALID_PARAMETER;
	}
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_POLL,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_dice(telebot_core_handler_t *core_h, long long int chat_id,
    bool disable_notification, int reply_to_message_id,
    const char *reply_markup, telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL)) {
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SEND_DICE,
	    mimes, index, response);
}

telebot_error_e
telebot_core_send_chat_action(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_SEND_CHAT_ACTION, mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_USER_PHOTOS,
	    mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_FILE, mimes,
	    index, response);
}
...
telebot_error_e
telebot_core_download_file(telebot_core_handler_t *core_h,
    const char *file_path, const char *out_file)
{
	int ret = TELEBOT_ERROR_NONE;
	if ((core_h == NULL) || (core_h->token == NULL) ||
	    (file_path == NULL) || (out_file == NULL))
		return TELEBOT_ERROR_INVALID_PARAMETER;
...
telebot_error_e
telebot_core_kick_chat_member(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	if (until_date > 0) {
		...
		mimes[index].type = TELEBOT_MIME_TYPE_DATA;
		snprintf(mimes[index].data, sizeof(mimes[index].data), "%ld",
		    until_date);
		++index;
	}

	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_KICK_CHAT_MEMBER, mimes, index, response);
}

telebot_error_e
telebot_core_unban_chat_member(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_UNBAN_CHAT_MEMBER, mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_RESTRICT_CHAT_MEMBER, mimes, index, response);
}

telebot_error_e
telebot_core_promote_chat_member(telebot_core_handler_t *core_h,
    ...
    bool can_promote_members, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_PROMOTE_CHAT_MEMBER, mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_SET_CHAT_ADMIN_TITLE, mimes, index, response);
}
    ...
    bool can_pin_messages, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_SET_CHAT_PERMISSIONS, mimes, index, response);
}
    ...
    long long int chat_id, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_EXPORT_CHAT_INVITE_LINK, mimes, index, response);
}

telebot_error_e
telebot_core_set_chat_photo(telebot_core_handler_t *core_h,
...
telebot_error_e
telebot_core_delete_chat_photo(telebot_core_handler_t *core_h,
    long long int chat_id, telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL)) {
		ERR("Handler, or token is NULL");
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_DELETE_CHAT_PHOTO, mimes, index, response);
}

telebot_error_e
telebot_core_set_chat_title(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_SET_CHAT_DESCRIPTION, mimes, index, response);
}

telebot_error_e
telebot_core_pin_chat_message(telebot_core_handler_t *core_h,
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_PIN_CHAT_MESSAGE, mimes, index, response);
}

telebot_error_e
telebot_core_unpin_chat_message(telebot_core_handler_t *core_h,
    long long int chat_id, telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL)) {
		ERR("Handler, or token is NULL");
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_UNPIN_CHAT_MESSAGE, mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_LEAVE_CHAT,
	    mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_CHAT, mimes,
	    index, response);
}

telebot_error_e
telebot_core_get_chat_admins(telebot_core_handler_t *core_h,
    long long int chat_id, telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL)) {
		ERR("Handler, or token is NULL");
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_CHAT_ADMINS,
	    mimes, index, response);
}
    ...
    long long int chat_id, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_GET_CHAT_MEMBERS_COUNT, mimes, index, response);
}

telebot_error_e
telebot_core_get_chat_member(telebot_core_handler_t *core_h,
    long long int chat_id, long long int user_id,
    telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL)) {
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_CHAT_MEMBER,
	    mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_SET_CHAT_STICKER_SET, mimes, index, response);
}
    ...
    long long int chat_id, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_DEL_CHAT_STICKER_SET, mimes, index, response);
}
    ...
    const char *url, int cache_time, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_ANSWER_CALLBACK_QUERY, mimes, index, response);
}

telebot_error_e
telebot_core_set_my_commands(telebot_core_handler_t *core_h,
    const char *commands, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_SET_MY_COMMANDS,
	    mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_GET_MY_COMMANDS,
	    NULL, 0, response);
}

telebot_error_e
telebot_core_edit_message_text(telebot_core_handler_t *core_h,
    long long int chat_id, int message_id, const char *inline_message_id,
    const char *text, const char *parse_mode, bool disable_web_page_preview,
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_EDIT_MESSAGE_TEXT, mimes, index, response);
}
    ...
    telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_EDIT_MESSAGE_CAPTION, mimes, index, response);
}
    ...
    const char *reply_markup, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h,
	    TELEBOT_METHOD_EDIT_MESSAGE_REPLY_MARKUP, mimes, index, response);
}

telebot_error_e
telebot_core_stop_poll(telebot_core_handler_t *core_h, long long int chat_id,
    int message_id, const char *reply_markup, telebot_core_response_t *response)
{
	if ((core_h == NULL) || (core_h->token == NULL)) {
		ERR("Handler or token is NULL");
		return TELEBOT_ERROR_INVALID_PARAMETER;
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_STOP_POLL,
	    mimes, index, response);
}

telebot_error_e
telebot_core_delete_message(telebot_core_handler_t *core_h,
    long long int chat_id, int message_id, telebot_core_response_t *response)
{
	...
	return telebot_core_curl_perform(core_h, TELEBOT_METHOD_DELETE_MESSAGE,
	    mimes, index, response);
}
