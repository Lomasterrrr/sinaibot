...
static const char *telebot_update_type_str[TELEBOT_UPDATE_TYPE_MAX] = {
	...
	"inline_query", "chonse_inline_result", "callback_query",
...
struct json_object *
telebot_parser_str_to_obj(char *data)
{
	return json_tokener_parse(data);
}

telebot_error_e
telebot_parser_get_updates(struct json_object *obj, telebot_update_t **updates,
    int *count)
{
	...
	*updates = result;
	...
	for (int index = 0; index < array_len; index++) {
		struct json_object *item = json_object_array_get_idx(array,
		    index);

		struct json_object *update_id = NULL;
		if (json_object_object_get_ex(item, "update_id", &update_id)) {
			result[index].update_id = json_object_get_int(
			    update_id);
		}

		struct json_object *message = NULL;
		if (json_object_object_get_ex(item, "message", &message)) {
			if (telebot_parser_get_message(message,
				&(result[index].message)) != TELEBOT_ERROR_NONE)
		...
		if (json_object_object_get_ex(item, "edited_message",
			&edited_message)) {
			if (telebot_parser_get_message(edited_message,
				&(result[index].edited_message)) !=
			    TELEBOT_ERROR_NONE)
				ERR("Failed to parse edited message of bot update");
			result[index].update_type =
			    TELEBOT_UPDATE_TYPE_EDITED_MESSAGE;
			continue;
		}

		struct json_object *channel_post = NULL;
		if (json_object_object_get_ex(item, "channel_post",
			&channel_post)) {
			if (telebot_parser_get_message(channel_post,
				&(result[index].channel_post)) !=
			    TELEBOT_ERROR_NONE)
		...
		if (json_object_object_get_ex(item, "edited_channel_post",
			&edited_channel_post)) {
			if (telebot_parser_get_message(edited_channel_post,
				&(result[index].edited_channel_post)) !=
			    TELEBOT_ERROR_NONE)
		...
		if (json_object_object_get_ex(item, "callback_query",
			&callback_query)) {
		...
		if (json_object_object_get_ex(item, "poll", &poll)) {
			if (telebot_parser_get_poll(poll,
				&(result[index].poll)) != TELEBOT_ERROR_NONE)
		...
		if (json_object_object_get_ex(item, "poll_answer",
			&poll_answer)) {
			if (telebot_parser_get_poll_answer(poll_answer,
				&(result[index].poll_anser)) !=
			    TELEBOT_ERROR_NONE)
...
telebot_error_e
telebot_parser_get_webhook_info(struct json_object *obj,
    telebot_webhook_info_t *info)
{
	...
	if (!json_object_object_get_ex(obj, "has_custom_certificate",
		&certificate)) {
	...
	if (!json_object_object_get_ex(obj, "pending_update_count",
		&update_count)) {
	...
	if (json_object_object_get_ex(obj, "last_error_date",
		&last_error_date)) {
		info->last_error_date = json_object_get_double(last_error_date);
	}

	struct json_object *last_error_message = NULL;
	if (json_object_object_get_ex(obj, "last_error_message",
		&last_error_message))
		info->last_error_message = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(last_error_message));
...
telebot_error_e
telebot_parser_get_user(struct json_object *obj, telebot_user_t *user)
{
	...
	if (json_object_object_get_ex(obj, "language_code", &language_code))
		user->language_code = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(language_code));

	struct json_object *can_join_groups = NULL;
	if (json_object_object_get_ex(obj, "can_join_groups", &can_join_groups))
		user->can_join_groups = json_object_get_boolean(
		    can_join_groups);

	struct json_object *can_read_all_group_messages = NULL;
	if (json_object_object_get_ex(obj, "can_read_all_group_messages",
		&can_read_all_group_messages))
		user->can_read_all_group_messages = json_object_get_boolean(
		    can_read_all_group_messages);

	struct json_object *supports_inline_queries = NULL;
	if (json_object_object_get_ex(obj, "supports_inline_queries",
		&supports_inline_queries))
		user->supports_inline_queries = json_object_get_boolean(
		    supports_inline_queries);

	return TELEBOT_ERROR_NONE;
}

static telebot_error_e
telebot_parser_get_users(struct json_object *obj, telebot_user_t **users,
    int *count)
{
	struct json_object *array = obj;
	int array_len = json_object_array_length(array);
	...
	*users = result;
	...
	for (index = 0; index < array_len; index++) {
		struct json_object *item = json_object_array_get_idx(array,
		    index);
		if (telebot_parser_get_user(item, &(result[index])) !=
		    TELEBOT_ERROR_NONE)
...
telebot_error_e
telebot_parser_get_chat(struct json_object *obj, telebot_chat_t *chat)
{
	...
	if (json_object_object_get_ex(obj, "title", &title))
		chat->title = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(title));

	struct json_object *username = NULL;
	if (json_object_object_get_ex(obj, "username", &username))
		chat->username = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(username));

	struct json_object *first_name = NULL;
	if (json_object_object_get_ex(obj, "first_name", &first_name))
		chat->first_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(first_name));

	struct json_object *last_name = NULL;
	if (json_object_object_get_ex(obj, "last_name", &last_name))
		chat->last_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(last_name));
	...
	if (json_object_object_get_ex(obj, "photo", &chat_photo)) {
		...
		if (telebot_parser_get_chat_photo(chat_photo, chat->photo) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "description", &description))
		chat->description = TELEBOT_SAFE_STRDUP(
	...
	if (json_object_object_get_ex(obj, "invite_link", &invite_link))
		chat->invite_link = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(invite_link));
	...
	if (json_object_object_get_ex(obj, "pinned_message", &pinned_message)) {
		...
		if (telebot_parser_get_message(pinned_message,
			chat->pinned_message) != TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "permissions", &permissions)) {
		chat->permissions = calloc(1,
		    sizeof(telebot_chat_permissions_t));
		if (telebot_parser_get_chat_permissions(permissions,
			chat->permissions) != TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "slow_mode_delay",
		&slow_mode_delay)) {
		chat->slow_mode_delay = json_object_get_int(slow_mode_delay);
	}

	struct json_object *sticker_set_name = NULL;
	if (json_object_object_get_ex(obj, "sticker_set_name",
		&sticker_set_name))
		chat->sticker_set_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(sticker_set_name));

	struct json_object *can_set_sticker_set = NULL;
	if (json_object_object_get_ex(obj, "can_set_sticker_set",
		&can_set_sticker_set))
		chat->can_set_sticker_set = json_object_get_boolean(
		    can_set_sticker_set);
...
telebot_error_e
telebot_parser_get_chat_admins(struct json_object *obj,
    telebot_chat_member_t **admins, int *count)
{
	...
	telebot_chat_member_t *result = calloc(array_len,
	    sizeof(telebot_chat_member_t));
	if (result == NULL)
		return TELEBOT_ERROR_OUT_OF_MEMORY;
...
telebot_error_e
telebot_parser_get_message(struct json_object *obj, telebot_message_t *msg)
{
	...
	if (json_object_object_get_ex(obj, "from", &from)) {
		...
		if (telebot_parser_get_user(from, msg->from) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "forward_from", &forward_from)) {
		...
		if (telebot_parser_get_user(forward_from, msg->forward_from) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "forward_from_chat",
		&forward_from_chat)) {
		msg->forward_from_chat = calloc(1, sizeof(telebot_chat_t));
		if (telebot_parser_get_chat(forward_from_chat,
			msg->forward_from_chat) != TELEBOT_ERROR_NONE) {
			ERR("Failed to get <forward from> from message object");
	...
	if (json_object_object_get_ex(obj, "forward_from_message_id",
		&forward_from_message_id))
		msg->forward_from_message_id = json_object_get_int(
		    forward_from_message_id);

	struct json_object *forward_signature = NULL;
	if (json_object_object_get_ex(obj, "forward_signature",
		&forward_signature))
		msg->forward_signature = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(forward_signature));

	struct json_object *forward_sender_name = NULL;
	if (json_object_object_get_ex(obj, "forward_sender_name",
		&forward_sender_name))
		msg->forward_sender_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(forward_sender_name));
	...
	if (json_object_object_get_ex(obj, "reply_to_message",
		&reply_to_message)) {
		msg->reply_to_message = calloc(1, sizeof(telebot_message_t));
		if (telebot_parser_get_message(reply_to_message,
			msg->reply_to_message) != TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "media_group_id", &media_group_id))
		msg->media_group_id = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(media_group_id));

	struct json_object *author_signature = NULL;
	if (json_object_object_get_ex(obj, "author_signature",
		&author_signature))
		msg->author_signature = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(author_signature));
	...
	if (json_object_object_get_ex(obj, "entities", &entities)) {
		if (telebot_parser_get_message_entities(entities,
			&(msg->entities),
			&(msg->count_entities)) != TELEBOT_ERROR_NONE)
			ERR("Failed to get <entities> from message object");
	}

	struct json_object *caption_entities = NULL;
	if (json_object_object_get_ex(obj, "caption_entities",
		&caption_entities)) {
		if (telebot_parser_get_message_entities(caption_entities,
	...
	if (json_object_object_get_ex(obj, "audio", &audio)) {
		...
		if (telebot_parser_get_audio(audio, msg->audio) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "document", &document)) {
		...
		if (telebot_parser_get_document(document, msg->document) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "photo", &photo)) {
		if (telebot_parser_get_photos(photo, &(msg->photos),
			&(msg->count_photos)) != TELEBOT_ERROR_NONE)
	...
	if (json_object_object_get_ex(obj, "video", &video)) {
		...
		if (telebot_parser_get_video(video, msg->video) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "voice", &voice)) {
		...
		if (telebot_parser_get_voice(voice, msg->voice) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "video_note", &video_note)) {
		...
		if (telebot_parser_get_video_note(video_note,
			msg->video_note) != TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "caption", &caption))
		msg->caption = TELEBOT_SAFE_STRDUP(
	...
	if (json_object_object_get_ex(obj, "contact", &contact)) {
		...
		if (telebot_parser_get_contact(contact, msg->contact) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "location", &location)) {
		...
		if (telebot_parser_get_location(location, msg->location) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "venue", &venue)) {
		...
		if (telebot_parser_get_venue(venue, msg->venue) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "poll", &poll)) {
		...
		if (telebot_parser_get_poll(poll, msg->poll) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "dice", &dice)) {
		...
		if (telebot_parser_get_dice(dice, msg->dice) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "new_chat_members", &ncm)) {
		if (telebot_parser_get_users(ncm, &(msg->new_chat_members),
			&(msg->count_new_chat_members)) != TELEBOT_ERROR_NONE)
			ERR("Failed to get <new_chat_members> from message object");
	}

	/* SINAIBOT FIX */
	struct json_object *lcm_obj = NULL;

	if (json_object_object_get_ex(obj, "left_chat_member", &lcm_obj)) {
		msg->left_chat_members = calloc(1, sizeof(telebot_user_t));
		if (!msg->left_chat_members)
			return TELEBOT_ERROR_OUT_OF_MEMORY;

		if (telebot_parser_get_user(lcm_obj,
			&(msg->left_chat_members[0])) == TELEBOT_ERROR_NONE)
			msg->count_left_chat_members = 1;
		else
	...
	if (json_object_object_get_ex(obj, "new_chat_title", &nct))
		msg->new_chat_title = TELEBOT_SAFE_STRDUP(
	...
	if (json_object_object_get_ex(obj, "new_chat_photo", &new_chat_photo)) {
		if (telebot_parser_get_photos(new_chat_photo,
			&(msg->new_chat_photos),
			&(msg->count_new_chat_photos)) != TELEBOT_ERROR_NONE)
			ERR("Failed to get <new_chat_photo> from message object");
	}

	struct json_object *del_chat_photo = NULL;
	if (json_object_object_get_ex(obj, "delete_chat_photo",
		&del_chat_photo))
		msg->delete_chat_photo = json_object_get_boolean(
		    del_chat_photo);
	...
	if (json_object_object_get_ex(obj, "pinned_message", &pinned_message)) {
		...
		if (telebot_parser_get_message(pinned_message,
			msg->pinned_message) != TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "connected_website",
		&connected_website))
		msg->connected_website = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(connected_website));

	// TODO: implement invoce, successful_payment, passport_data,
	// reply_markup
...
telebot_error_e
telebot_parser_get_message_entity(struct json_object *obj,
    telebot_message_entity_t *entity)
{
	...
	if (json_object_object_get_ex(obj, "user", &user)) {
		...
		if (telebot_parser_get_user(user, entity->user) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "language", &language))
		entity->language = TELEBOT_SAFE_STRDUP(
...
telebot_error_e
telebot_parser_get_message_entities(struct json_object *obj,
    telebot_message_entity_t **entities, int *count)
{
	...
	telebot_message_entity_t *result = calloc(array_len,
	    sizeof(telebot_message_entity_t));
	if (result == NULL)
		return TELEBOT_ERROR_OUT_OF_MEMORY;
	...
	*entities = result;
	...
	for (int index = 0; index < array_len; index++) {
		struct json_object *item = json_object_array_get_idx(array,
		    index);
		if (telebot_parser_get_message_entity(item, &(result[index])) !=
		    TELEBOT_ERROR_NONE)
			ERR("Failed to parse user from users array");
	}
...
telebot_error_e
telebot_parser_get_photo(struct json_object *obj, telebot_photo_t *photo)
{
...
static telebot_error_e
telebot_parser_get_photos(struct json_object *obj, telebot_photo_t **photos,
    int *count)
{
	struct json_object *array = obj;
	int array_len = json_object_array_length(array);
	...
	*photos = result;
	...
	for (int index = 0; index < array_len; index++) {
		struct json_object *item = json_object_array_get_idx(array,
		    index);
		if (telebot_parser_get_photo(item, &(result[index])) !=
		    TELEBOT_ERROR_NONE)
			ERR("Failed to parse user from users array");
	}
...
telebot_error_e
telebot_parser_get_audio(struct json_object *obj, telebot_audio_t *audio)
{
	...
	if (!json_object_object_get_ex(obj, "file_unique_id",
		&file_unique_id)) {
		ERR("Object is not audio type, file_unique_id not found");
		TELEBOT_SAFE_FREE(audio->file_id);
		return TELEBOT_ERROR_OPERATION_FAILED;
	}
	audio->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	if (json_object_object_get_ex(obj, "thumb", &thumb)) {
		...
		if (telebot_parser_get_photo(thumb, audio->thumb) !=
		    TELEBOT_ERROR_NONE) {
...
telebot_error_e
telebot_parser_get_document(struct json_object *obj,
    telebot_document_t *document)
{
	...
	document->file_id = TELEBOT_SAFE_STRDUP(
	...
	document->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	if (json_object_object_get_ex(obj, "file_name", &file_name))
		document->file_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(file_name));

	struct json_object *mime_type = NULL;
	if (json_object_object_get_ex(obj, "mime_type", &mime_type))
		document->mime_type = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(mime_type));
...
telebot_error_e
telebot_parser_get_video(struct json_object *obj, telebot_video_t *video)
{
	...
	video->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	struct json_object *mime_type = NULL;
	if (json_object_object_get_ex(obj, "mime_type", &mime_type))
		video->mime_type = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(mime_type));
...
telebot_error_e
telebot_parser_get_animation(struct json_object *obj,
    telebot_animation_t *animation)
{
	...
	animation->file_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_id));
	...
	animation->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	if (json_object_object_get_ex(obj, "thumb", &thumb)) {
		...
		if (telebot_parser_get_photo(thumb, animation->thumb) !=
		    TELEBOT_ERROR_NONE) {
	...
	struct json_object *mime_type = NULL;
	if (json_object_object_get_ex(obj, "mime_type", &mime_type))
		animation->mime_type = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(mime_type));
...
telebot_error_e
telebot_parser_get_video_note(struct json_object *obj,
    telebot_video_note_t *video_note)
{
	...
	video_note->file_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_id));
	...
	video_note->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	if (json_object_object_get_ex(obj, "thumb", &thumb)) {
		...
		if (telebot_parser_get_photo(thumb, video_note->thumb) !=
		    TELEBOT_ERROR_NONE) {
...
telebot_error_e
telebot_parser_get_voice(struct json_object *obj, telebot_voice_t *voice)
{
	...
	voice->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	struct json_object *mime_type = NULL;
	if (json_object_object_get_ex(obj, "mime_type", &mime_type))
		voice->mime_type = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(mime_type));
...
telebot_error_e
telebot_parser_get_contact(struct json_object *obj, telebot_contact_t *contact)
{
	...
	contact->phone_number = TELEBOT_SAFE_STRDUP(
	...
	contact->first_name = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(first_name));

	struct json_object *last_name = NULL;
	if (json_object_object_get_ex(obj, "last_name", &last_name))
		contact->last_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(last_name));
	...
	if (json_object_object_get_ex(obj, "vcard", &vcard))
		contact->vcard = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(vcard));

	return TELEBOT_ERROR_NONE;
}

telebot_error_e
telebot_parser_get_location(struct json_object *obj,
...
telebot_error_e
telebot_parser_get_venue(struct json_object *obj, telebot_venue_t *venue)
{
	...
	if (json_object_object_get_ex(obj, "location", &location)) {
		...
		if (telebot_parser_get_location(location, venue->location) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "foursquare_id", &foursquare_id))
		venue->foursquare_id = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(foursquare_id));

	struct json_object *foursquare_type = NULL;
	if (json_object_object_get_ex(obj, "foursquare_type", &foursquare_type))
		venue->foursquare_type = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(foursquare_type));

	return TELEBOT_ERROR_NONE;
}

telebot_error_e
telebot_parser_get_poll_option(struct json_object *obj,
    telebot_poll_option_t *option)
{
...
telebot_error_e
telebot_parser_get_poll_answer(struct json_object *obj,
    telebot_poll_answer_t *answer)
{
	...
	answer->option_ids = calloc(answer->count_option_ids,
	    sizeof(*(answer->option_ids)));
	...
	for (int index = 0; index < answer->count_option_ids; index++)
		answer->option_ids[index] = json_object_get_int(
		    json_object_array_get_idx(option_ids, index));

	return TELEBOT_ERROR_NONE;
}

telebot_error_e
telebot_parser_get_poll(struct json_object *obj, telebot_poll_t *poll)
{
	...
	if (!json_object_object_get_ex(obj, "total_voter_count",
		&total_voter_count)) {
	...
	if (!json_object_object_get_ex(obj, "allows_multiple_answers",
	...
	poll->allows_multiple_answers = json_object_get_boolean(
	    allows_multiple_answers);
	...
	poll->options = calloc(poll->count_options,
	    sizeof(telebot_poll_option_t));
	...
	for (int index = 0; index < poll->count_options; index++)
		telebot_parser_get_poll_option(
		    json_object_array_get_idx(options, index),
		    &(poll->options[index]));
...
telebot_error_e
telebot_parser_get_dice(struct json_object *obj, telebot_dice_t *dice)
{
...
telebot_error_e
telebot_parser_get_user_profile_photos(struct json_object *obj,
	...
	for (int i = 0; i < total; i++) {
		...
		for (int j = 0; j < subtotal; j++) {
			struct json_object *photo =
			    json_object_array_get_idx(item, j);
			ret |= telebot_parser_get_photo(photo,
			    &(photos->photos[j][i]));
		}
	}
...
telebot_error_e
telebot_parser_get_file(struct json_object *obj, telebot_file_t *file)
{
	...
	file->file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(file_unique_id));
	...
	file->file_path = TELEBOT_SAFE_STRDUP(
...
telebot_error_e
telebot_parser_get_callback_query(struct json_object *obj,
    telebot_callback_query_t *cb_query)
{
	...
	if (json_object_object_get_ex(obj, "from", &from)) {
		...
		if (telebot_parser_get_user(from, cb_query->from) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "message", &message)) {
		...
		if (telebot_parser_get_message(message, cb_query->message) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "inline_message_id",
		&inline_message_id))
		cb_query->inline_message_id = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(inline_message_id));

	struct json_object *chat_instance = NULL;
	if (json_object_object_get_ex(obj, "chat_instance", &chat_instance))
		cb_query->chat_instance = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(chat_instance));

	struct json_object *data = NULL;
	if (json_object_object_get_ex(obj, "data", &data))
		cb_query->data = TELEBOT_SAFE_STRDUP(
	...
	if (json_object_object_get_ex(obj, "game_short_name", &game_short_name))
		cb_query->game_short_name = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(game_short_name));

	return TELEBOT_ERROR_NONE;
}

telebot_error_e
telebot_parser_get_chat_photo(struct json_object *obj,
    telebot_chat_photo_t *photo)
{
	...
	photo->small_file_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(small_file_id));

	struct json_object *small_file_unique_id = NULL;
	if (!json_object_object_get_ex(obj, "small_file_unique_id",
		&small_file_unique_id)) {
		ERR("Object is not chat photo type, small_file_unique_id not found");
		TELEBOT_SAFE_FREE(photo->small_file_id);
		return TELEBOT_ERROR_OPERATION_FAILED;
	}
	photo->small_file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(small_file_unique_id));
	...
	photo->big_file_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(big_file_id));

	struct json_object *big_file_unique_id = NULL;
	if (!json_object_object_get_ex(obj, "big_file_unique_id",
		&big_file_unique_id)) {
	...
	photo->big_file_unique_id = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(big_file_unique_id));

	return TELEBOT_ERROR_NONE;
}

telebot_error_e
telebot_parser_get_chat_member(struct json_object *obj,
    telebot_chat_member_t *member)
{
	...
	if (json_object_object_get_ex(obj, "user", &user)) {
		...
		if (telebot_parser_get_user(user, member->user) !=
		    TELEBOT_ERROR_NONE) {
	...
	if (json_object_object_get_ex(obj, "custom_title", &custom_title))
		member->custom_title = TELEBOT_SAFE_STRDUP(
		    json_object_get_string(custom_title));
	...
	if (json_object_object_get_ex(obj, "can_post_messages",
		&can_post_messages))
		member->can_post_messages = json_object_get_boolean(
		    can_post_messages);

	struct json_object *can_edit_messages = NULL;
	if (json_object_object_get_ex(obj, "can_edit_messages",
		&can_edit_messages))
		member->can_edit_messages = json_object_get_boolean(
		    can_edit_messages);

	struct json_object *can_delete_messages = NULL;
	if (json_object_object_get_ex(obj, "can_delete_messages",
		&can_delete_messages))
		member->can_delete_messages = json_object_get_boolean(
		    can_delete_messages);

	struct json_object *can_restrict_members = NULL;
	if (json_object_object_get_ex(obj, "can_restrict_members",
		&can_restrict_members))
		member->can_restrict_members = json_object_get_boolean(
		    can_restrict_members);

	struct json_object *can_promote_members = NULL;
	if (json_object_object_get_ex(obj, "can_promote_members",
		&can_promote_members))
		member->can_promote_members = json_object_get_boolean(
		    can_promote_members);

	struct json_object *can_change_info = NULL;
	if (json_object_object_get_ex(obj, "can_change_info", &can_change_info))
		member->can_change_info = json_object_get_boolean(
		    can_change_info);

	struct json_object *can_invite_users = NULL;
	if (json_object_object_get_ex(obj, "can_invite_users",
		&can_invite_users))
		member->can_invite_users = json_object_get_boolean(
		    can_invite_users);

	struct json_object *can_pin_messages = NULL;
	if (json_object_object_get_ex(obj, "can_pin_messages",
		&can_pin_messages))
		member->can_pin_messages = json_object_get_boolean(
		    can_pin_messages);
	...
	if (json_object_object_get_ex(obj, "can_send_media_messages",
		&can_send_media_messages))
		member->can_send_media_messages = json_object_get_boolean(
		    can_send_media_messages);

	struct json_object *can_send_polls = NULL;
	if (json_object_object_get_ex(obj, "can_send_polls", &can_send_polls))
		member->can_send_polls = json_object_get_boolean(
		    can_send_polls);

	struct json_object *can_send_other_messages = NULL;
	if (json_object_object_get_ex(obj, "can_send_other_messages",
		&can_send_other_messages))
		member->can_send_other_messages = json_object_get_boolean(
		    can_send_other_messages);

	struct json_object *can_add_web_page_previews = NULL;
	if (json_object_object_get_ex(obj, "can_add_web_page_previews",
		&can_add_web_page_previews))
		member->can_add_web_page_previews = json_object_get_boolean(
		    can_add_web_page_previews);
...
telebot_error_e
telebot_parser_get_chat_permissions(struct json_object *obj,
    telebot_chat_permissions_t *permissions)
{
	...
	if (json_object_object_get_ex(obj, "can_send_messages",
		&can_send_messages))
		permissions->can_send_messages = json_object_get_boolean(
		    can_send_messages);

	struct json_object *can_send_media_messages = NULL;
	if (json_object_object_get_ex(obj, "can_send_media_messages",
		&can_send_media_messages))
		permissions->can_send_media_messages = json_object_get_boolean(
		    can_send_media_messages);

	struct json_object *can_send_polls = NULL;
	if (json_object_object_get_ex(obj, "can_send_polls", &can_send_polls))
		permissions->can_send_polls = json_object_get_boolean(
		    can_send_polls);

	struct json_object *can_send_other_messages = NULL;
	if (json_object_object_get_ex(obj, "can_send_other_messages",
		&can_send_other_messages))
		permissions->can_send_other_messages = json_object_get_boolean(
		    can_send_other_messages);
	...
	if (json_object_object_get_ex(obj, "can_add_web_page_previews",
		...
		permissions->can_add_web_page_previews =
		    json_object_get_boolean(can_add_web_page_previews);

	struct json_object *can_change_info = NULL;
	if (json_object_object_get_ex(obj, "can_change_info", &can_change_info))
		permissions->can_change_info = json_object_get_boolean(
		    can_change_info);

	struct json_object *can_invite_users = NULL;
	if (json_object_object_get_ex(obj, "can_invite_users",
		&can_invite_users))
		permissions->can_invite_users = json_object_get_boolean(
		    can_invite_users);

	struct json_object *can_pin_messages = NULL;
	if (json_object_object_get_ex(obj, "can_pin_messages",
		&can_pin_messages))
		permissions->can_pin_messages = json_object_get_boolean(
		    can_pin_messages);
...
telebot_error_e
telebot_parser_get_bot_command(struct json_object *obj,
    telebot_bot_command_t *botcmd)
{
	...
	botcmd->description = TELEBOT_SAFE_STRDUP(
	    json_object_get_string(description));
...
telebot_error_e
telebot_parser_get_array_bot_command(struct json_object *obj,
    telebot_bot_command_t **cmds, int *count)
{
	int ret = TELEBOT_ERROR_NONE;
	...
	telebot_bot_command_t *result = calloc(array_len,
	    sizeof(telebot_bot_command_t));
	if (result == NULL)
		return TELEBOT_ERROR_OUT_OF_MEMORY;
...
telebot_error_e
telebot_parser_get_response_parameters(struct json_object *obj,
    telebot_response_paramters_t *resp_param)
{
	...
	if (json_object_object_get_ex(obj, "migrate_to_chat_id",
		&migrate_to_chat_id))
		resp_param->migrate_to_chat_id = json_object_get_int(
		    migrate_to_chat_id);
